...
export const Component = ({ orbitItems, stageSize = 320, imageSize = 60 }: ComponentProps) => {
  ...
  React.useEffect(() => {
    ...
    // image lift-in
    setTimeout(() => animate(imgs, { top: 0 }, transition), 250)
    ...
    setTimeout(() => animate(orbitPlacementSequence), 700)
      ...
      >
        {orbitItems.map((item, i) => (
          ...
          >
            <motion.img
              data-arm-image
              className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 rounded-full object-cover shadow-lg shadow-black/40 border border-white/10"
              style={{
                width: imageSize,
                height: imageSize,
                opacity: i === 0 ? 1 : 0,
